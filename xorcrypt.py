...
def read_shellcode(path, text_mode):
    if (text_mode): 
        return parse_text_shellcode(read_text_file(path))
    else:
...
def parse_text_shellcode(content): 
    ...
    if "\\x" in compact:
        ...
        print("[+] Reading text shellcode (\\xNN format)")
    ...
    if "0x" in compact.lower():
        ...
        print("[+] Reading text shellcode (0xNN format)")
        return bytes(int(p, 16) for p in pairs)

    # 3) Pure hextring: 4831c050...
    if re.fullmatch(r"[0-9a-fA-F]+", compact) and len(compact) % 2 == 0:
        print("[+] Reading text shellcode (raw hex string)")
...
def parse_args():
    parser = argparse.ArgumentParser(description="XOR encrypt shellcode")
    ...
    key_group.add_argument("--key-str")
